package com.idsr.mobile.models;

public class Immunization {
	private String BCGdate;
	private String HEPAwithdate;
	private String HEPAmoredate;
	private String OPV1date;
	private String OPV2date;
	private String OPV3date;
	private String PENTA1date;
	private String PENTA2date;
	private String PENTA3date;
	private String PCV1date;
	private String PCV2date;
	private String PCV3date;
	private String MCV1date;
	private String MCV2date;
	private String Dengue1date;
	private String Dengue2date;
	private String Dengue3date;

	public Immunization() {
	}

	public String getBCGdate() {
		return BCGdate;
	}

	public void setBCGdate(String BCGdate) {
		this.BCGdate = BCGdate;
	}

	public String getHEPAwithdate() {
		return HEPAwithdate;
	}

	public void setHEPAwithdate(String HEPAwithdate) {
		this.HEPAwithdate = HEPAwithdate;
	}

	public String getHEPAmoredate() {
		return HEPAmoredate;
	}

	public void setHEPAmoredate(String HEPAmoredate) {
		this.HEPAmoredate = HEPAmoredate;
	}

	public String getOPV1date() {
		return OPV1date;
	}

	public void setOPV1date(String OPV1date) {
		this.OPV1date = OPV1date;
	}

	public String getOPV2date() {
		return OPV2date;
	}

	public void setOPV2date(String OPV2date) {
		this.OPV2date = OPV2date;
	}

	public String getOPV3date() {
		return OPV3date;
	}

	public void setOPV3date(String OPV3date) {
		this.OPV3date = OPV3date;
	}

	public String getPENTA1date() {
		return PENTA1date;
	}

	public void setPENTA1date(String PENTA1date) {
		this.PENTA1date = PENTA1date;
	}

	public String getPENTA2date() {
		return PENTA2date;
	}

	public void setPENTA2date(String PENTA2date) {
		this.PENTA2date = PENTA2date;
	}

	public String getPENTA3date() {
		return PENTA3date;
	}

	public void setPENTA3date(String PENTA3date) {
		this.PENTA3date = PENTA3date;
	}

	public String getPCV1date() {
		return PCV1date;
	}

	public void setPCV1date(String PCV1date) {
		this.PCV1date = PCV1date;
	}

	public String getPCV2date() {
		return PCV2date;
	}

	public void setPCV2date(String PCV2date) {
		this.PCV2date = PCV2date;
	}

	public String getPCV3date() {
		return PCV3date;
	}

	public void setPCV3date(String PCV3date) {
		this.PCV3date = PCV3date;
	}

	public String getMCV1date() {
		return MCV1date;
	}

	public void setMCV1date(String MCV1date) {
		this.MCV1date = MCV1date;
	}

	public String getMCV2date() {
		return MCV2date;
	}

	public void setMCV2date(String MCV2date) {
		this.MCV2date = MCV2date;
	}

	public String getDengue1date() {
		return Dengue1date;
	}

	public void setDengue1date(String dengue1date) {
		Dengue1date = dengue1date;
	}

	public String getDengue2date() {
		return Dengue2date;
	}

	public void setDengue2date(String dengue2date) {
		Dengue2date = dengue2date;
	}

	public String getDengue3date() {
		return Dengue3date;
	}

	public void setDengue3date(String dengue3date) {
		Dengue3date = dengue3date;
	}
}
